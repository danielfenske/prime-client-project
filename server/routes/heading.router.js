    ...
    if (req.isAuthenticated()) {
        ...
        pool.query(sqlText)
            .then((result) => {
                ...
                console.log('error in item_heading/item_code router GETting all line items and related info', error);
                res.sendStatus(500)
            })
    } else {
        res.sendStatus(403);
    ...
    if (req.isAuthenticated()) {
        ...
        pool.query(sqlText, valueArray)
            .then((result) => {
                ...
                console.log('error posting a new line item', error);
                res.sendStatus(500);
            })
    } else {
        res.sendStatus(403)
...
router.put('/item/update/ft_inches', async (req, res) => {
    ...
    if (req.isAuthenticated()) {
        try {
            ...
            const sqlText =
                `UPDATE "item_heading"
                 SET "ft" = $1, "inches" = $2, "qty" = $3, "price_per_pricing_unit" = $4, "message" = $5
                 WHERE "id" = $6;`;

            const valueArray = [req.body.ft, req.body.inches, req.body.qty, req.body.price_per_pricing_unit, req.body.message, req.body.heading_item_id];
            ...
            // rounded_measurement_per_unit gets updated based on the partner rounding type
            const sqlTextRounding =
                `UPDATE "item_heading"
                 SET "rounded_measurement_per_unit" = CASE WHEN "partner"."rounding_type" = 1 THEN "measurement_per_unit"
